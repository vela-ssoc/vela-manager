...
func (inm *intom) BindRoute(anon, auth *ship.RouteGroupBuilder) {
	anon.Route("/intom/:param/*path").Any(inm.Warp(inm.Forward))
}

func (inm *intom) Warp(fn func(*ship.Context, *model.Minion) error) ship.Handler {
...
func (inm *intom) Forward(c *ship.Context, mon *model.Minion) error {
	path := c.Param("path")
	w, r := c.ResponseWriter(), c.Request()
	query := r.URL.RawQuery
	op := opurl.MgtIntom(c.Method(), mon.BrokerID, mon.ID, path).WithQuery(query)

	inm.hub.ForwardM(op, w, r)
